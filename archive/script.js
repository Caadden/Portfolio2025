...
// Config: swipe threshold in pixels (touchmove -> touchend delta)
...
window.addEventListener('DOMContentLoaded', () => {
    // Time-based background video logic (robust)
    const video = document.querySelector('.bg-video');
    const source = video ? video.querySelector('source') : null;
    function clearThemeClasses() {
        document.body.classList.remove('theme-morning', 'theme-afternoon', 'theme-evening', 'theme-night', 'bg-afternoon2');
    }
    ...
    function setBackgroundForHour(hour) {
        ...
        if (hour >= 5 && hour < 12) {
            ...
            // Add a per-video body class so we can style per-background (afternoon1/afternoon2)
        ...
        if (source.getAttribute('src') !== videoFile) {
            ...
            try {
                video.load();
                // attempt to play (some browsers require user gesture; this is non-blocking)
    ...
    if (video && source) {
        ...
        // Re-evaluate every 5 minutes in case user leaves page open across thresholds
    ...
    setupHamburgerMenu();
    function updateSwipeArrowVisibility(page) {
    ...
    window.addEventListener('touchend', (e) => {
        if (touchStartY === null || scrollCooldown) return;
            ...
            if (touchEndY !== null) {
                ...
                if (Math.abs(deltaY) > SWIPE_MIN_DISTANCE) { // Minimum swipe distance (configurable)
                    scrollCooldown = true;
